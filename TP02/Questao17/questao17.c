...
{
    if(jog1->altura == jog2->altura)
    {
        return strcmp(jog1->nome, jog2->nome) > 0;
    }
    else
    {
        return jog1->altura > jog2->altura;
    }
}

/*Procedimento que constroi o heap*/

void construir(Jogador **jogadores, int tam)
{
    for (int i = tam; i > 1 && compare(jogadores[i],jogadores[i/2]); i = i / 2)
    {
        swap(jogadores[i], jogadores[i/2]);
    }
}

/*Procedimento para achar o maior filho de um certo nó*/

int getMaiorFilho(Jogador **jogadores, int i, int tam)
{
    int filho;
    if(2*i == tam || compare(jogadores[2*i], jogadores[2*i + 1]))
    {
        filho =  2*i;
    }
    else
...
/*Procedimento que reconstroi o heap apos a troca do primeiro com o ultimo elemento*/
...
void reconstruir(Jogador **jogadores, int tam)
{
    ...
    while(i <= (tam/2))
    {
        int filho = getMaiorFilho(jogadores, i, tam);
        if(compare(jogadores[filho], jogadores[i]))
        {
...
void heapsort(Jogador **jogadores_param, int n, int k)
{
    ...
    for (int i = 0; i < n; i++)
    {
        jogadores[i + 1] = jogadores_param[i];
        //imprimir(jogadores[i + 1]); 
    }
    ...
    for(int i = k + 1; i <= n; i++)
    {
        if(compare(jogadores[1],jogadores[i]))
        {
            swap(jogadores[i], jogadores[1]);
            reconstruir(jogadores,k);
        }
    }
...
int main()
{
    ...
    heapsort(jogadores_para_ordenar, i-1, 10);
