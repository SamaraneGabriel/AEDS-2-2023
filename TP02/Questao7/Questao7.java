...
class Jogador {
    ...
    public static void insertionSort(Lista lista, int[] contador_mov, int[] contador_comp) throws Exception {
        int n = lista.size();
        for (int i = 1; i < n; ++i) {
            Jogador key = lista.get(i);
            int j = i - 1;

            while (j >= 0 && lista.get(j).getAnoNascimento() > key.getAnoNascimento()) {
                contador_comp[0]++;
        ...
        for (int i = 1; i < n; ++i) {
            ...
            while (j >= 0 && lista.get(j).getNome().compareTo(key.getNome()) > 0 && lista.get(j).getAnoNascimento() == key.getAnoNascimento()){
                contador_comp[0]++;
                lista.set(j+1, lista.get(j));
                contador_mov[0]++;
                j = j - 1;
...
public class Questao7 extends Jogador{
    ...
    public static void main(String[] args) throws Exception{
        ...
        insertionSort(Jogadores, contador_mov, contador_comp);
