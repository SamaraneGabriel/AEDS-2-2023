...
class Jogador {
    ...
    /*
     * Procedimento de ordenação parcial utilizando como base o algoritmo QuickSort
     * 
     * @params: Lista de jogadores a serem ordenados, int k delemitador de
     * parcialidade
     */

    public static void quickSortParcial(Lista jogadores, int esq, int dir, int k) throws Exception {
        Jogador pivo = jogadores.get((esq + dir) / 2);
        int i = esq;
        int j = dir;

        while (i <= j) {
            while (compare(jogadores.get(i), pivo) < 0) {
                i++;
            }

            while (compare(jogadores.get(j), pivo) > 0) {
                j--;
            }
    ...
    public static int compare(Jogador jog1, Jogador jog2) {
        int comp = jog1.estadoNascimento.compareTo(jog2.estadoNascimento);
        if (comp == 0) {
            return jog1.nome.compareTo(jog2.nome);
...
public class Questao18 extends Jogador {
    ...
    public static void main(String[] args) throws Exception {
        ...
        quickSortParcial(Jogadores, 0, Jogadores.size() - 1,10);
