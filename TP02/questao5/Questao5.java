...
class Lista {  //Lista para o tipo Jogador
    ...
    public Jogador get(int index) throws Exception{
        if(index < 0 || index >= JogadorAMANHO_MAX+1){        //Checa posicao pedida
...
class Jogador {
    ...
    public static Lista selecao_nome(Lista Jogadores, int[] contador_mov, int[] contador_comp) throws Exception{
        for(int i = 0; i < Jogadores.size() - 1; i++){
            int menor = i;
            for(int j = (i + 1); j < Jogadores.size(); j++){
                String nome_j = Jogadores.get(j).nome;
                String nome_menor = Jogadores.get(menor).nome;
                contador_comp[0]++;
                if(nome_menor.compareTo(nome_j) > 0){
                    Jogadores.swap(menor,j);
                    contador_mov[0] += 3;
                }
            }
        }
...
public class Questao5 extends Jogador{
    ...
    public static void main(String[] args) throws Exception{
        ...
        selecao_nome(Jogadores, contador_mov, contador_comp);
        ...
        String nomeArquivo = "725052_insercao.txt";
